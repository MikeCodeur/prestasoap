<?php

define( '_PS_VERSION_', 1 );

/**
 * Prestashop SOAP Connector
 *
 * Prestashop SOAP Connector 
 *
 * @package    prestasoap
 * @subpackage modules
 * @author     Mickael cabanas (cabanas.mickael|at|gmail.com)
 * @copyright  2012 Mickael Cabanas
 * @license    http://www.gnu.org/copyleft/gpl.html GNU/GPL
 * @version    $Id:$
 */

 /** loading framework **/
include_once('../Commons.php');

	
	/**
    * This function getByID
	* (expose as WS)
    * @param
    * @return 
   */
	function GetOrder_cart_ruleById($params) {
		
		$WSRestresource = 'order_discounts';
		$resource = 'order_cart_rules';
		$resourceSingle = 'order_cart_rule';
		return GetById($params,$resource,$resourceSingle,$WSRestresource);
		
	}
	/**
    * This function GetALL
	* (expose as WS)
    * @param
    * @return 
   */
	function GetOrder_cart_rules($params) {
	
		$WSRestresource = 'order_discounts';
		$resource = 'order_cart_rules';
		$resourceSingle = 'order_cart_rule';
		return GetList($params,$resource,$resourceSingle,$WSRestresource);
		
	}
	
	/**
    * This function Crete Object
	* (expose as WS)
    * @param
    * @return 
   */
	function CreateOrder_cart_rule($params) {
		
		$WSRestresource = 'order_discounts';
		$resource = 'order_cart_rules';
		$resourceSingle = 'order_cart_rule';
		return CreateMapper($params,$resource,$resourceSingle,$WSRestresource);
	}
	
	/**
    * This function Update Object
	* (expose as WS)
    * @param
    * @return 
   */
	function UpdateOrder_cart_rule($params) {
		
		$WSRestresource = 'order_discounts';
		$resource = 'order_cart_rules';
		$resourceSingle = 'order_cart_rule';
		return UpdateMapper($params,$resource,$resourceSingle,$WSRestresource);
	
	}
	
	/**
    * This function Delete Object
	* (expose as WS)
    * @param
    * @return 
   */
	function DeleteOrder_cart_rule($params) {
		
		$WSRestresource = 'order_discounts';
		$resource = 'order_cart_rules';
		$resourceSingle = 'order_cart_rule';
		return DeleteMapper($params,$resource,$resourceSingle,$WSRestresource);
	
	}

	
	
	
	
	$soap_ws_custom_on = WSDL_SERVICES_ON;
	/* SOAP SETTINGS */
	if ($soap_ws_custom_on==1){
		$soap_ws_custom_cache_on = WSDL_CACHE_ON;
		ini_set("soap.wsdl_cache_enabled", $soap_ws_custom_cache_on); // wsdl cache settings
		$options = array('soap_version' => SOAP_1_2);
		
		/** SOAP SERVER **/
		$server = new SoapServer(WSDL_ORDER_DISCOUNTS);
		
		/* Add Functions */
		$server->addFunction("GetOrder_cart_rules");
		$server->addFunction("GetOrder_cart_ruleById");
		$server->addFunction("CreateOrder_cart_rule");
		$server->addFunction("UpdateOrder_cart_rule");
		$server->addFunction("DeleteOrder_cart_rule");

		$server->handle();
		
	}else{
		echoXmlMessageWSDisabled('Order_discount');
	}
?> 