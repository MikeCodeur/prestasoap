...
include_once('../Commons.php');
   ...
   */
	...
	if ($soap_ws_custom_on==1){
		...
		/** SOAP SERVER **/
		$server = new SoapServer(WSDL_GROUPS);
		
		/* Add Functions */
		$server->addFunction("GetGroups");
		$server->addFunction("GetGroupById");
		$server->addFunction("CreateGroup");
		$server->addFunction("UpdateGroup");
		$server->addFunction("DeleteGroup");

		$server->handle();
		
	}else{
		echoXmlMessageWSDisabled('Group');
	}
?> 